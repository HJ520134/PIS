...
{
    public interface IMaterialNormalDemandRepository : IRepository<Material_Normal_Demand>
    {
        IQueryable<MaterialNormalDemandDTO> GetInfo(MaterialNormalDemandDTO searchModel, Page page, out int totalcount);
        IQueryable<MaterialNormalDemandDTO> GetDetailInfo(MaterialNormalDemandDTO searchModel, Page page, out int totalcount);
        List<MaterialNormalDemandDTO> DoExportFunction(Material_Normal_Demand MND);
    }
    public class MaterialNormalDemandRepository: RepositoryBase<Material_Normal_Demand>, IMaterialNormalDemandRepository
    {
        public MaterialNormalDemandRepository(IDatabaseFactory databaseFactory)
            : base(databaseFactory)
        {
        }

        public IQueryable<MaterialNormalDemandDTO> GetInfo(MaterialNormalDemandDTO searchModel, Page page, out int totalcount)
        {
            var query1 = from M in DataContext.Material_Normal_Demand
                             //fky2017/11/13
                        // where M.Status_UID != 414
                         ...
                         select new
                         {
                             MaxUid = g.Max(m => m.Material_Normal_Demand_UID)
                         };
            if (searchModel.Status_UID != 0)
                query1 = from M in DataContext.Material_Normal_Demand
            ...
            var query = from M in DataContext.Material_Normal_Demand
                        ...
                        join temp in query1 on M.Material_Normal_Demand_UID equals temp.MaxUid
                        select new MaterialNormalDemandDTO
                        {
                            Material_Normal_Demand_UID = M.Material_Normal_Demand_UID,
        ...
        public IQueryable<MaterialNormalDemandDTO> GetDetailInfo(MaterialNormalDemandDTO searchModel, Page page, out int totalcount)
        {
            var query = from M in DataContext.Material_Normal_Demand
                         ...
                         select new MaterialNormalDemandDTO
                         {
                             ...
                             Safe_Stock_Qty = M.Safe_Stock_Qty,
                             Existing_Stock_Qty = M.Existing_Stock_Qty,
        ...
        public List<MaterialNormalDemandDTO> DoExportFunction(Material_Normal_Demand MND)
        {
            var query = from M in DataContext.Material_Normal_Demand
                        join plantbom in DataContext.System_OrganizationBOM
                        on M.BG_Organization_UID equals plantbom.ChildOrg_UID
            ...
            query = query.Where(m => m.BG_Organization_UID == MND.BG_Organization_UID & m.Demand_Date == MND.Demand_Date
                                                        &m.FunPlant_Organization_UID==MND.FunPlant_Organization_UID);
