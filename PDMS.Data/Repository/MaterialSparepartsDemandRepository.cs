...
{
    public interface IMaterialSparepartsDemandRepository : IRepository<Material_Spareparts_Demand>
    {
        IQueryable<MaterialSparepartsDemandDTO> GetInfo(MaterialSparepartsDemandDTO searchModel, Page page, out int totalcount);
        IQueryable<MaterialSparepartsDemandDTO> GetDetailInfo(MaterialSparepartsDemandDTO searchModel, Page page, out int totalcount);
        List<MaterialSparepartsDemandDTO> DoExportFunction(Material_Spareparts_Demand MND);
    }
    public class MaterialSparepartsDemandRepository: RepositoryBase<Material_Spareparts_Demand>, IMaterialSparepartsDemandRepository
    {
        public MaterialSparepartsDemandRepository(IDatabaseFactory databaseFactory)
            : base(databaseFactory)
        {
        }

        public IQueryable<MaterialSparepartsDemandDTO> GetInfo(MaterialSparepartsDemandDTO searchModel, Page page, out int totalcount)
        {
            var query1 = from M in DataContext.Material_Spareparts_Demand
                             //fky2017/11/13
                        // where M.Status_UID != 414
                         ...
                         select new
                         {
                             MaxUid = g.Max(m => m.Material_Spareparts_Demand_UID)
                         };
            if (searchModel.Status_UID != 0)
            {
                query1 = from M in DataContext.Material_Spareparts_Demand
            ...
            var query = from M in DataContext.Material_Spareparts_Demand
                        ...
                        join temp in query1 on M.Material_Spareparts_Demand_UID equals temp.MaxUid
                        select new MaterialSparepartsDemandDTO
                        {
                            Material_Spareparts_Demand_UID = M.Material_Spareparts_Demand_UID,
        ...
        public IQueryable<MaterialSparepartsDemandDTO> GetDetailInfo(MaterialSparepartsDemandDTO searchModel, Page page, out int totalcount)
        {
            var query = from M in DataContext.Material_Spareparts_Demand
        ...
        public List<MaterialSparepartsDemandDTO> DoExportFunction(Material_Spareparts_Demand MND)
        {
            var query = from M in DataContext.Material_Spareparts_Demand
                        join plantbom in DataContext.System_OrganizationBOM
                        on M.BG_Organization_UID equals plantbom.ChildOrg_UID
