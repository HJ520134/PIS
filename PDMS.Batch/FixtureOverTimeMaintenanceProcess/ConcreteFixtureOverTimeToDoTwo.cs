...
{
    public class ConcreteFixtureOverTimeToDoTwo : ABSFixtureOverTimeToDo
    {
        public override void Request(string request)
        {
            ...
            if (!CacheHelper.HasException())
            {
                ...
                {
                    //模具排程信息统计成功，写入排程日志表
                    new ExceptionEmailManager().InsertLogInfo(ConstConstants.FixtureNotMaintenanceBatch, true, null);
                }
                else if (todo != null)
                {
