...
!function ($) {
    ...
    var Page = function (element, options) {
        ...
        var init = function (obj) {
            ...
            var infoWarpper = $('<div class="col-xs-12 col-sm-6"></div>');
            var pageWarpper = $('<div class="col-xs-12 col-sm-6 text-right"></div>');

            infoWarpper.append(that.$info.hide())
            pageWarpper.append(that.$jump.hide()).append(that.$size.hide()).append(that.$page.hide());

            that.$element.append(infoWarpper);
            that.$element.append(pageWarpper);
    ...
    var renderPages = function (pageIndex, pageSize, total, pageBtnCount, firstBtnText, lastBtnText, prevBtnText, nextBtnText, showFirstLastBtn) {
        ...
        var html = [];
        //add by Rock 2017-06-22 修改tabley样式表头居中----start
        $("table th").removeClass('text-right');
        //add by Rock 2017-06-22 修改tabley样式表头居中----end
